...
def prune_model_width(model, hidden_size, num_heads, embed_size):
    prune_heads(model, num_heads)
    prune_mlp(model, hidden_size)
    prune_embeddings(model, embed_size)
    
def prune_model_depth(model, tokenizer, num_layers_to_remove):
    pruned_blocks(model, tokenizer, num_layers_to_remove)
...
def prune_embeddings(model, new_embed_dim:int) -> None:
    ...
    for i, module in enumerate(model_blocks):
        ...
        if i == 0:
            model.transformer.wte = pruned_embedding(model.transformer.wte, idx_ln1, model.device, dim=1)
            model.transformer.wpe = pruned_embedding(model.transformer.wpe, idx_ln1, model.device, dim=1)
        if i>0:
    ...
    model_blocks[-1].mlp.c_proj = pruned_layer(model_blocks[-1].mlp.c_proj, idx_lnf, model.device, dim=1)
